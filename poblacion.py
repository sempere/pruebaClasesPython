...
def lee_fichero(fichero):
    ...
    with open("hospital.csv", encoding='utf-8') as f:
        ...
        for i, registro in enumerate(lector):
            id = int(registro[0])
            date = datetime.strptime(registro[10], "%d/%m/%Y")
            print(date)
            if (i > 0):
                break
            #tupla = Registro(nombre = name, codigo = codigo, año = anyo, censo = num_habitantes)
            #registros.append(tupla)
    return registros
# Mostremos los 10 primeros registros
     
############################################################################################
poblaciones = lee_fichero("hospital.csv")
...
def calcula_paises(poblaciones):
    ...
    for poblacion in poblaciones:
        paises.append(poblacion.nombre)
    paises.sort()    
    return set(paises)
...
Grafica = namedtuple("Grafica", "año, pepito")
def filtra_por_pais(poblaciones, pais):
    ...
    listaRetorno = []
    for poblacion in poblaciones:
        if poblacion.nombre == pais or poblacion.codigo == pais :
            t = Grafica(año = poblacion.año, pepito = poblacion.censo)
            listaRetorno.append(t)
...
def muestra_evolucion_poblacion(poblaciones, pais):
    ...
    poblacionesFiltradas = filtra_por_pais(poblaciones, pais)
    
    plt.title("Hola")
    l_anyos = []
    l_habitantes = []
    print(poblacionesFiltradas)
    for poblacion in poblacionesFiltradas :
        l_anyos.append(poblacion[0])
        l_habitantes.append(poblacion.pepito)
    plt.plot(l_anyos,l_habitantes)
    plt.show()
...
def muestra_comparativa_paises_anyo(poblaciones, anyo, paises):
    ...
    plt.title("asdad")
    indice = range(0,6)
    plt.bar(indice, [10,20,30,30,40,50])
    plt.xticks(indice, ["Spaiassasasn", "Xssdasd", "X33", "Xs", "Xww", "Xee"], fontsize=8)
    plt.show()
